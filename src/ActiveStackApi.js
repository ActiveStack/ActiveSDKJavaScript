angular.module('ActiveStack.Api', ['ActiveStack.Model','ActiveStack.Client','ActiveStack.Config','ActiveStack.Domain'], function($provide) {
    $provide.factory('ActiveStackApi', function($log, ActiveStackModel, ActiveStackClient, ActiveStackConfig, ActiveStackDomain) {
        var ActiveStackApi = function() {
...
//                        deferred.resolve(message.result);
            ...
            this.authenticateWithOAuth = function(providerId){
                ...
                if (providerId && ActiveStackConfig.oauthProviders.hasOwnProperty(providerId)) {
                    oauthProvider = ActiveStackConfig.oauthProviders[providerId];
                }
                else {
...
//                        deferred.resolve(message.result);
            ...
            this.setupClient = function(){
                ...
                client.on('sync#PushDeleteResponse', function(evt) {
                    ...
                    for (var i = 0; i < objectList.length; i++) {
                        var o = objectList[i];
                        ActiveStackModel.removeObject(o);
                    };
                });
                client.on('sync#PushCWUpdateResponse', function(evt) {
                    var ob = ActiveStackModel.findObjectByClassIdPair(evt.data.classIdPair);
                    if(ob){
                        ob[evt.data.fieldName] = evt.data.value;
                        ActiveStackModel.notifyChanged(evt.data);
                    }
                });
            ...
            this.putObject = function(o, callback) {
                ...
                client.sendRequest("putObject", request, function(result) {
                    if (result.result === true) {
                        ActiveStackModel.filterIncoming(o);
                        result = true;
                    } else {
                        result = false;
                    }
            ...
            /**
             * Utility function to ensure that a certain path of the object ActiveStackModel is loaded
             *
             * @param obj
             * @param path
